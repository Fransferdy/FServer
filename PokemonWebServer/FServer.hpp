...
/*
Send Email using SMTP, WITH TLS and port 587
*/
static void sendMail(std::string user, std::string pass, std::string smtpserver, std::string to, std::string fromAlias, std::string subject, std::string body, std::string tlsmode="- starttls", int port = 587)
{
	std::string funct;
	...
	funct = "start CMail.exe -host:" + user + ":" + pass + "@" + smtpserver + ":" + std::to_string(port) + +" " + tlsmode + " -to:" + to + " -from:" + user + ":" + fromAlias + " \"-subject:" + subject + "\" \"-body:" + body + "\" -head:Content-Type:text/html";
	system(funct.c_str());
}

/*
Send Email using SMTP and CONF File
*/
static int sendMail(std::string to, std::string subject, std::string body)
{
	std::string user, pass, smtpserver, fromAlias, port, tlsmode;
	std::ifstream mailconf;
	...
	mailconf.open("mailconf.txt");
	if (mailconf.is_open())
	{
		std::getline(mailconf,smtpserver);
		std::getline(mailconf, port);
		std::getline(mailconf, user);
		std::getline(mailconf, fromAlias);
		std::getline(mailconf, pass);
		std::getline(mailconf, tlsmode);
		mailconf.close();
		std::string funct;
		funct = "CMail.exe -host:" + user + ":" + pass + "@" + smtpserver + ":" + port + "" + tlsmode + " -to:" + to + " -from:" + user + ":" + fromAlias + " \"-subject:" + subject + "\" \"-body:" + body + "\" -head:Content-Type:text/html";
...
protected:
	...
	virtual int answer(void *cls, struct MHD_Connection *connection, const char *url, const char *method, const char *version, const char *upload_data, size_t *upload_data_size, void **con_cls)
	{
		...
		try
		{
			...
			answerPage->setUpPage(connection, &session->parameters, &session->newcookies, &session->cookies);
