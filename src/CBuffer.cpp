...
void CBuffer::StreamWrite(void *in, int size)
{
	...
	if(writepos+size >= BuffSize)
	{
		...
		if((tempData = (char*)realloc(data, BuffSize)) == NULL)return;
		data = tempData;
	}
...
void CBuffer::reduceBuffer(int size)
{
	char *tempdata;
	int tempBuffSize = count-size + 30;
	tempdata = (char*)malloc(BuffSize);
	if (tempdata == NULL) return;

	memcpy(tempdata, data+size, count-size);
	if(data!=NULL)
		free(data);
...
int CBuffer::replaceint(int a, int pos)
{
	memcpy(&data[pos], &a, sizeof(int));
	return 1;
}
